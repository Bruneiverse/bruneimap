...
sch_df <- read_csv("data-raw/schools/data/School listing.csv")
...
sch_df <- read_excel("data-raw/schools/data/school listing.xlsx", 1)
...
for (i in sheets) {
  df <- read_excel("data-raw/schools/data/moe2018.xlsx", i$sheet)
  colnames(df) <- df[2, ]
  df <- df %>%
    slice(3:nrow(df)) %>%
...
for (i in 4:6) {
  df <- read_excel("data-raw/schools/data/moe2018.xlsx", i)
  colnames(df) <- df[2, ]
  df <- df %>%
    slice(3:nrow(df))
